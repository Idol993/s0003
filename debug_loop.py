...
for step in range(5):
    ...
    print(f"  total_aux_loss type: {type(out.total_aux_loss)}, requires_grad: {out.total_aux_loss.requires_grad if isinstance(out.total_aux_loss, torch.Tensor) else 'N/A'}")
    print(f"  total_aux_loss: {out.total_aux_loss}")
    g = out.gate_output
    print(f"  gate_output loss_info: {g.loss_info}")
    print(f"  gate_output routing_stats: {g.routing_stats}")
